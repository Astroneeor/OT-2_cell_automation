...
# define separate color map for slots or labware
slot_colors = {
    'slot 1': Fore.LIGHTBLUE_EX,
    'slot 2': Fore.LIGHTMAGENTA_EX,
    'slot 3': Fore.LIGHTCYAN_EX,
    'slot 4': Fore.LIGHTGREEN_EX,
    'slot 5': Fore.LIGHTYELLOW_EX,
    'slot 12': Fore.LIGHTRED_EX
}

labware_colors = {
    'Tip Rack': Fore.CYAN,
    'Tube Rack': Fore.YELLOW,
    'Well Plate': Fore.GREEN
}

labware_color_map = {
            "Opentrons OT-2 96 Tip Rack 1000 µL": Fore.CYAN,
            "16.8mm_lifted_24wp_flat": Fore.GREEN,
            "Corning 96 Well Plate 360 µL Flat": Fore.LIGHTMAGENTA_EX,
            "Opentrons 10 Tube Rack with Falcon 4x50 mL, 6x15 mL Conical": Fore.LIGHTYELLOW_EX
        }

aspirate_wells_per_labware = {}
dispense_wells_per_labware = {}

volume_from_labware = {}
tip_count = 0

class ColorizingStream:
    def __init__(self, stream):
        self.stream = stream

    def write(self, text):
        global aspirate_wells_per_labware, dispense_wells_per_labware
        global volume_from_labware, tip_count

        # Process incoming text line-by-line
        for line in text.splitlines(keepends=True):
            if "Picking up tip" in line:    
                tip_count += 1

            m_transfer = re.search(r"Transferring\s+([\d\.]+)\s+from\s+(\b[A-H]\d{1,2}\b)\s+of\s+(.+?)\s+on slot", line)
            
            if m_transfer:
                vol       = float(m_transfer.group(1))
                well      = m_transfer.group(2)
                src_lab   = m_transfer.group(3)
                # initialize nested dict if needed
                volume_from_labware.setdefault(src_lab, {})
                # add volume for that specific well
                volume_from_labware[src_lab].setdefault(well, 0.0)
                volume_from_labware[src_lab][well] += vol

            # Check for Aspirate lines and capture "<well> of <labware_name>"
            m_asp = re.search(r"Aspirating.*?from\s+(\b[A-H]\d{1,2}\b)\s+of\s+(.+?)\s+on slot", line)
            if m_asp:
                well = m_asp.group(1)
                labware = m_asp.group(2)
                aspirate_wells_per_labware.setdefault(labware, set()).add(well)

            # Check for Dispense lines and capture "<well> of <labware_name>"
            m_disp = re.search(                r"Dispensing.*?into\s+(\b[A-H]\d{1,2}\b)\s+of\s+(.+?)\s+on slot", line)
            if m_disp:
                well = m_disp.group(1)
                labware = m_disp.group(2)
                dispense_wells_per_labware.setdefault(labware, set()).add(well)

            #Colorize “Picking up tip”, “Dropping tip”, “Blowing out”, etc.
            colored = line
            colored = re.sub(r"(Picking up tip)", Fore.CYAN + r"\1" + Style.RESET_ALL, colored)
            colored = re.sub(r"(Dropping tip)", Fore.CYAN + r"\1" + Style.RESET_ALL, colored)
            colored = re.sub(r"(Aspirating)", Fore.BLUE + r"\1" + Style.RESET_ALL, colored)
            colored = re.sub(r"(Dispensing)", Fore.GREEN + r"\1" + Style.RESET_ALL, colored)
            colored = re.sub(r"(Transferring)", Fore.YELLOW + r"\1" + Style.RESET_ALL, colored)
            colored = re.sub(r"(Blowing out)", Fore.MAGENTA + r"\1" + Style.RESET_ALL, colored)
            colored = re.sub(r"(Touching tip)", Fore.LIGHTWHITE_EX + r"\1" + Style.RESET_ALL, colored)
            colored = re.sub(r"(Pausing robot operation)", Fore.LIGHTRED_EX + r"\1" + Style.RESET_ALL, colored)

            # 4) Highlight well IDs wherever they appear (optional)
            colored = re.sub(r"\b([A-H]\d{1,2})\b", Fore.LIGHTWHITE_EX + r"\1" + Style.RESET_ALL, colored)

            # 5) Finally, print this (possibly colorized) line
            self.stream.write(colored)

    def flush(self):
        self.stream.flush()

sys.stdout = ColorizingStream(sys.stdout)

import os
import webbrowser
from datetime import datetime

def write_html_report(aspirate_wells, dispense_wells, volumes, tip_count):
    """Emit an HTML file with your summary tables and open it in the default browser."""
    # Build rows for the per‐labware well‐usage table
    per_labware_rows = []
    all_labware = set(aspirate_wells) | set(dispense_wells)
    for lw in sorted(all_labware):
        a = ", ".join(sorted(aspirate_wells.get(lw, []))) or "&lt;none&gt;"
        d = ", ".join(sorted(dispense_wells.get(lw, []))) or "&lt;none&gt;"
        per_labware_rows.append(f"<tr><td>{lw}</td><td>{a}</td><td>{d}</td></tr>")

    # Build rows for your Falcon rack well volumes
    falcon = "Opentrons 10 Tube Rack with Falcon 4x50 mL, 6x15 mL Conical"
    vol_rows = []
    for well, v in sorted(volumes.get(falcon, {}).items()):
        vol_rows.append(f"<tr><td>{well}</td><td>{int(v)}</td></tr>")
...
<head>
  <meta charset="utf-8">
  <title>OT-2 Simulation Summary</title>
  <style>
...
<body>
  <h1>OT-2 Simulation Summary</h1>
  <p><em>Generated on {datetime.now().isoformat(sep=' ', timespec='seconds')}</em></p>

  <h2>Per-Labware Well Usage</h2>
  <table>
    <thead>
      <tr><th>Labware</th><th>Aspirated Wells</th><th>Dispensed Wells</th></tr>
    </thead>
    <tbody>
      {"".join(per_labware_rows) or "<tr><td colspan='3'>&lt;no actions detected&gt;</td></tr>"}
    </tbody>
  </table>

  <h2>Falcon Rack Volumes</h2>
  <table>
    <thead><tr><th>Well</th><th>Volume (µL)</th></tr></thead>
    <tbody>
      {"".join(vol_rows) or "<tr><td colspan='2'>&lt;no transfers detected&gt;</td></tr>"}
    </tbody>
  </table>

  <h2>Tip Count</h2>
  <p><strong>Total tips picked up:</strong> {tip_count}</p>
</body>
</html>
"""

    out_path = os.path.abspath("ot2_simulation_report.html")
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(html)

    # Open it in the browser (file:// URL)
    # webbrowser.open(f"file://{out_path}")


def _print_final_table():
    """After the simulation completes, print:
       1) For each labware: which wells were aspirated/dispensed
       2) For each source labware: how many µL were withdrawn total
       3) How many tips the run used
    """
    print("\n" + "="*60)
    print("Per-Labware Well Usage Summary")
    print("-" * 60)
    print(f"{'Labware':<35} | Aspirated Wells    | Dispensed Wells")
    print("-" * 60)

    # 1) Table of wells per labware
    all_labware = set(aspirate_wells_per_labware) | set(dispense_wells_per_labware)
    if not all_labware:
        print("  <no aspirate/dispense actions detected>")
    else:
        for lw in sorted(all_labware):
            a_wells = sorted(aspirate_wells_per_labware.get(lw, set()))
            d_wells = sorted(dispense_wells_per_labware.get(lw, set()))
            a_str = ", ".join(a_wells) if a_wells else "<none>"
            d_str = ", ".join(d_wells) if d_wells else "<none>"
            print(f"{lw:<35} | {a_str:<15} | {d_str}")

    print("-" * 60)

    # 2) Reservoir volumes needed (only for Falcon tube rack)
    falcon_rack = "Opentrons 10 Tube Rack with Falcon 4x50 mL, 6x15 mL Conical"
    print("\nTotal Liquid Required for Falcon Rack:")
    print("-" * 60)
    print(f"{'Well':<4} | Volume (µL)")
    print("-" * 60)

    # grab only that rack’s wells (empty dict if none)
    falcon_wells = volume_from_labware.get(falcon_rack, {})

    if not falcon_wells:
        print(f"<No transfers detected for {falcon_rack}>")
    else:
        for well, tot in sorted(falcon_wells.items()):
            print(f"{well:<4} | {int(tot)}")

    # 3) Tip‐count summary (unchanged)
    print("\nTip Count Summary:")
    print("-" * 60)
    print(f"Total tips picked up: {tip_count}")
    print("="*60 + "\n")
