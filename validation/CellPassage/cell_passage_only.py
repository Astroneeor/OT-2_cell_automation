...
def add_parameters(parameters):
    ...
    parameters.add_string(
        display_name="96-Well Plate Quadrant",
        variable_name="quadrant",
        default="1",
        choices=[
            {"display_name": "Quadrant 1 (A1–H6)", "value": "1"},
            {"display_name": "Quadrant 2 (A7–H12)", "value": "2"},
            {"display_name": "Quadrant 3 (I1–P6)", "value": "3"},
            {"display_name": "Quadrant 4 (I7–P12)", "value": "4"},
        ],
        description="Select select the quadrant you plan to use for 96wp"
    )
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    QUADRANT = protocol.params.quadrant
    ...
    for i in range(4):
        if QUADRANT == "2": pico_quad.append(pico_plate.rows()[i][6:])
        elif QUADRANT == '3': pico_quad.append(pico_plate.rows()[i+4][:6])
        elif QUADRANT == '4': pico_quad.append(pico_plate.rows()[i+4][6:])
        else: pico_quad.append(pico_plate.rows()[i][:6])
